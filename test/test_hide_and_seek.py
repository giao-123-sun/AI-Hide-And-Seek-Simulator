...
def input(key):
    ...
    # 新增：截图逻辑
    if key == 'f2' and screenshot_count < 3:
        screenshot_count += 1
        
        # --- THE CORRECT FIX ---
        # Call the screenshot method on the 'app' instance
        app.screenshot(name_prefix=f'screenshot_{screenshot_count}')
        
        print(f'Screenshot {screenshot_count} saved!')
        # 可以在屏幕上给一个短暂提示
        Text(text=f'Screenshot {screenshot_count} Saved!', origin=(0,0), scale=2, color=color.lime, duration=1)
